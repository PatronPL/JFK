namespace Synthesis
{
    ...
    internal sealed class Rewriter : CSharpSyntaxRewriter
    {
        ...
        {
            string defaultAccess = "private"; // domyślny modyfikator dostępu w przypadku jego braku
            int i = 0;
            var tokenTmp = node.GetFirstToken().ToString();
            var listOfTokens = node.ChildTokens().ToList();
            while (tokenTmp != "private" && tokenTmp != "protected" && tokenTmp != "internal" && tokenTmp != "public")
            {
                i++;
                if (i < listOfTokens.Count)
                {
                    tokenTmp = listOfTokens[i].ToString();
                }
                else
                {
                    tokenTmp = defaultAccess;
                }
                
            }
            if (tokenTmp == "private" || tokenTmp == "protected" || tokenTmp == "internal" || tokenTmp == "public")
            {
                list.Add(tokenTmp);
                nodeList.Add(node);
                switch (tokenTmp)
